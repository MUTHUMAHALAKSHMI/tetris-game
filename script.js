...
function arenaSweep() {
  ...
  if (lines >= level * 10) {
    level++;
    dropInterval = Math.max(200, dropInterval - 100); // speed up
  }
...
document.addEventListener("keydown", e => {
  ...
  else if (e.key.toLowerCase()==="p") paused=!paused, !paused && update();
  else if (e.key.toLowerCase()==="r") resetGame(), update();
});

const arena = createMatrix(COL, ROW);

const player = { pos: {x:0,y:0}, matrix: null };
const nextPiece = { matrix: createPiece(pieces[(pieces.length*Math.random())|0]) };
